...
class GooglePositioning(object):
    ...
    def read_insia_speed_limits(self):
        ...
        filePath = "/Users/carlosdeantonio/Documents/UNIVERSIDAD/GPS/Practica 3/Python/coordenadasInsia.txt"
    ...
    # Creation of KML file to use in Google Earth
    def get_earth_kml(self, Longitude, Latitude):

        with open("position.kml", "r+") as positionKML:
            positionKML.write("""<?xml version="1.0" encoding="UTF-8"?>
                               <kml xmlns="http://earth.google.com/kml/2.1">
                               <Document>
                               <Placemark>
                               <name>Google Earth - Live GPS Positioning</name>
                               <Point>
                                 <coordinates>%f,%f,0</coordinates>
                               </Point>
                               </Placemark></Document></kml>""" % (Longitude, Latitude))
    ...
    def check_near(self):
        ...
        if speed == 0:
            ...
            return 1, speed
        elif (self.speedMatrix[self.SPEED_POSITION][2] - (
                self.speedMatrix[self.SPEED_POSITION][2] * 0.1)) < speed < (
    ...
    def check_all(self):
        ...
        if speed == 0:
            ...
            return 1, speed
        elif (float(self.speedMatrix[lowestPosition][2]) - (
                float(self.speedMatrix[lowestPosition][2]) * 0.1)) < speed < (
                float(self.speedMatrix[lowestPosition][2]) + (float(self.speedMatrix[lowestPosition][2]) * 0.1)):
            return 2, speed
        elif speed > (
                float(self.speedMatrix[lowestPosition][2]) + (float(self.speedMatrix[lowestPosition][2]) * 0.1)):
        ...
        try:
            ...
            while True:
                ...
                if data[0] == "$GPGGA":
                    ...
                    self.get_earth_kml(degreeLongitude, degreeLatitude)
